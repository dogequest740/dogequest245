...
const getAttackFromState = (state: unknown) => {
  ...
  const equipment = (state as { equipment?: Record<string, { bonuses?: { attack?: number } } | null> }).equipment ?? {};
  let bonus = 0;
...
serve(async (req) => {
  ...
  if (loadError) {
    return new Response(JSON.stringify({ error: "Failed to load boss" }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  if (!boss) {
    return new Response(JSON.stringify({ error: "Boss row missing" }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
